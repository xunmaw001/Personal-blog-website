<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'"  label="相册分类" prop="xiangcefenlei">
        ...
        <div v-else>
          <el-form-item class="input" label="相册分类" prop="xiangcefenlei">
...
export default {
  ...
  methods: {
	...
	this.ruleForm.xiangceneirong = this.ruleForm.xiangceneirong.replace(reg,'../../../ssmckr8l/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `xiangcefenxiang/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.xiangcefenxiangCrossAddOrUpdateFlag = false;
                  this.parent.search();
